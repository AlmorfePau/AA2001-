    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          ...
          <div>
            <h1 className="text-[40px] font-black text-[#1e293b] tracking-tight leading-none">Global Systems Console</h1>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100">
              ...
              <div className="space-y-4">
                {['SYS_USER_01', 'SYS_USER_02', 'SYS_USER_03'].map((sysUser, i) => (
                  <div key={i} className="flex items-center justify-between p-5 bg-slate-50/40 rounded-2xl border border-slate-100">
                    <div className="flex items-center gap-4">
                      <div className="w-5 h-5 rounded bg-white border border-slate-200"></div>
                      <span className="text-[10px] font-black text-[#1e293b] tracking-wider">{sysUser}</span>
                    </div>
                    <button className="text-[9px] font-black text-blue-600 uppercase tracking-widest hover:underline">MODIFY</button>
                  </div>
                ))}
              </div>
            </div>

            {/* System Root Audit Section */}
            <div className="bg-[#0f172a] rounded-[2.5rem] p-10 shadow-xl border border-slate-800 text-white">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-8 h-8 flex items-center justify-center">
                  <Terminal className="w-5 h-5 text-blue-400" />
                </div>
                <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">SYSTEM ROOT AUDIT</h3>
              </div>
              
              <div className="space-y-4 font-mono text-[10px]">
                <div className="flex items-center gap-3">
                  <span className="text-emerald-400">[OK]</span>
                  <span className="text-slate-300">DB_HANDSHAKE_SUCCESS</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-blue-400">[INFO]</span>
                  <span className="text-slate-300">SEC_KEY_ROTATION_INIT</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-emerald-400">[OK]</span>
                  <span className="text-slate-300">CORE_HEARTBEAT_STABLE</span>
                </div>
