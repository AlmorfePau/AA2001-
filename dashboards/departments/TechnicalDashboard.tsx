...
const TechnicalDashboard: React.FC<Props> = ({ user, validatedStats, pendingTransmissions, transmissionHistory, announcements, onTransmit }) => {
  ...
  const handleTransmit = () => {
    ...
    const transmission: Transmission = {
      ...
      projectReport: formData.projectReport, attachments: formData.attachments
    };
    setTimeout(() => {
      ...
      {showSuccess && (
        <div className="fixed top-24 right-8 z-[100] animate-in slide-in-from-right-full fade-in duration-500">
          <div className="bg-[#0b1222] text-white px-6 py-4 rounded-[1.5rem] shadow-2xl border border-emerald-500/30 flex items-center gap-4">
            ...
            <div><p className="text-[11px] font-black uppercase tracking-widest mb-1">Logs Transmitted</p><p className="text-[9px] font-bold text-emerald-400 uppercase tracking-tighter">Archived for Supervisor Review</p></div>
          </div>
        </div>
      )}
                ...
                {[
                  ...
                  <div key={i} className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 space-y-2">
                    ...
                    <div className="flex items-end gap-1">
                      <p className="text-xl font-black text-slate-900">{row.final}</p>
                      <p className="text-[8px] font-bold text-slate-400 uppercase mb-1">of {row.weight}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-visible flex flex-col min-h-[650px]">
              ...
              {activeStep === 1 && (
                <div className="space-y-6 animate-in slide-in-from-left-4 fade-in duration-500">
                  ...
                  <div className="grid grid-cols-2 gap-6 pt-4">
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Start Time</label><input type="datetime-local" className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-sm text-black" value={formData.startTime} onChange={e => setFormData({...formData, startTime: e.target.value})} /></div>
                    <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">End Time</label><input type="datetime-local" className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-sm text-black" value={formData.endTime} onChange={e => setFormData({...formData, endTime: e.target.value})} /></div>
                  </div>
                </div>
              )}
              {activeStep === 2 && (
            ...
            <div className="bg-slate-50 p-6 flex items-center justify-between border-t border-slate-100 rounded-b-[2.5rem]">
              ...
              {activeStep < 4 ? (
                <button onClick={() => setActiveStep(prev => prev + 1)} disabled={(activeStep === 1 && !isStep1Complete) || (activeStep === 2 && !isStep2Complete) || (activeStep === 3 && !isStep3Complete)} className={`flex items-center gap-2 px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-all ${((activeStep === 1 && isStep1Complete) || (activeStep === 2 && isStep2Complete) || (activeStep === 3 && isStep3Complete)) ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>Continue <ChevronRight className="w-4 h-4" /></button>
              ) : (
